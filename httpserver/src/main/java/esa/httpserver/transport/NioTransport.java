...
public class NioTransport implements Transport {
    ...
    static {
        USE_UNPOOLED_ALLOCATOR =
                SystemPropertyUtil.getBoolean("esa.httpserver.useUnpooledAllocator", false);
        if (Loggers.logger().isDebugEnabled()) {
            Loggers.logger().debug("-Desa.httpserver.useUnpooledAllocator: {}", USE_UNPOOLED_ALLOCATOR);
        }
    }
